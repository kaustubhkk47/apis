...
def get_categories_details(categoryIds = ""):
    ...
    try:
    	if categoryIds == "":
    		...
    		categoriesArr = [int(e) if e.isdigit() else e for e in categoryIds.split(",")]
    		if len(categoriesArr) == 1:
    			...
    			closeDBConnection()
    			return customResponse("2XX", {"categories": multiple_products_parser(categoriesWithProducts)})
    		else:
